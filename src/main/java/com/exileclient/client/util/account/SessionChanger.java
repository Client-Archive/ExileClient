package com.exileclient.client.util.account;
/*
-----------------------------
    SessionChanger.java
      made by Savrien
...
// NOTE: You must change the session variable in Minecraft.java to public
...
public class SessionChanger {
    ...
    //Online mode
    //Checks if your already logging in to the account.
    public void setUser(String email, String password) {
        ExileClient.getInstance().getSocketController().sendServerRequest("start_client", Minecraft.getMinecraft().thePlayer.getGameProfile().getId() + ":false");
        if(!Minecraft.getMinecraft().getSession().getUsername().equals(email) || Minecraft.getMinecraft().getSession().getToken().equals("0")){
            ...
            try {
                ...
                setSession(session);
                ExileClient.getInstance().getSocketController().sendServerRequest("start_client", Minecraft.getMinecraft().thePlayer.getGameProfile().getId() + ":true");
            }
            catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public void setUserMicrosoft(String email, String password) {
        ...
        try {
            ...
            this.setSession(new Session(acc.getProfile().getName(), acc.getProfile().getId(), acc.getAccessToken(), "legacy"));
