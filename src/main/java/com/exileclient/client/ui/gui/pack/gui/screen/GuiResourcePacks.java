package com.exileclient.client.ui.gui.pack.gui.screen;

import com.exileclient.client.ExileClient;
import com.exileclient.client.module.data.setting.types.ChoiceSetting;
import com.exileclient.client.ui.gui.pack.gui.components.ResourcePacksFolder;
import com.exileclient.client.ui.gui.pack.gui.components.ResourcePacksWatcher;
import com.exileclient.client.ui.gui.pack.gui.components.list.GuiResourcePacksAvailable;
import com.exileclient.client.ui.gui.pack.gui.components.list.GuiResourcePacksSelected;
...
public class GuiResourcePacks extends GuiScreen {
    ...
    public void initGui() {
        ...
        boolean wide = (Boolean) ExileClient.getInstance().getGlobalSettings().widePackMenu.getValue();
        ...
        if (ExileClient.getInstance().getGlobalSettings().widePackMenu.getValue()) {
        ...
        this.searchBar.setVisible(this.activePackFolder == null && ExileClient.getInstance().getGlobalSettings().showPackSearchBar.getValue());
        ...
        this.clearGlassButton = new GuiOptionButton(100,
                ...
                114,
                20,
                "Clear Glass: " + ExileClient.getInstance().getGlobalSettings().clearGlass.getValue());
        this.buttonList.add(this.clearGlassButton);
        this.redStringButton = new GuiOptionButton(101,
                this.width / 2 + 118, this.height - 71,
                86,
                20,
                "Red String: " + (ExileClient.getInstance().getGlobalSettings().redString.getValue() ? "ON" : "OFF"));
        ...
        this.backgroundButton = new GuiOptionButton(
                ...
                20,
                "Background: " + (ExileClient.getInstance().getGlobalSettings().transparentBackground.getValue() ? "TRANSPARENT" : "NORMAL"));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.mc.theWorld != null && (Boolean) ExileClient.getInstance().getGlobalSettings().transparentBackground.getValue()) {
...
//        super.drawGradientRect(0, this.height - 45, this.width, this.height - 40, 0, -16777216);
        ...
        if (this.searchBar.getVisible() && !this.searchBar.isFocused() && this.searchBar.getText().isEmpty()) {
            super.drawString(this.fontRendererObj, EnumChatFormatting.GRAY + "" + EnumChatFormatting.ITALIC + "Search...", (Boolean) ExileClient.getInstance().getGlobalSettings().widePackMenu.getValue() ? 14 : this.width / 2 - 200, this.height - 65, 0xFFFFFF);
        }
    }
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == this.buttonFolder.id) {
            ...
            ChoiceSetting clearGlass = ExileClient.getInstance().getGlobalSettings().clearGlass;
            ...
            boolean redString = ExileClient.getInstance().getGlobalSettings().redString.getValue();
            this.redStringButton.displayString = "Red String: " + (!redString ? "ON" : "OFF");
            ExileClient.getInstance().getGlobalSettings().redString.setValue(!redString);
            this.mc.renderGlobal.loadRenderers();
        } else if (button.id == this.backgroundButton.id) {
            boolean transparentBackground = ExileClient.getInstance().getGlobalSettings().transparentBackground.getValue();
            this.backgroundButton.displayString = "Background: " + (!transparentBackground ? "TRANSPARENT" : "NORMAL");
            ExileClient.getInstance().getGlobalSettings().transparentBackground.setValue(!transparentBackground);
            this.mc.renderGlobal.loadRenderers();
        }
    }
    ...
    public void setAvailablePacks(ResourcePacksFolder packFolder) {
        ...
        int searchBarHeight = ExileClient.getInstance().getGlobalSettings().showPackSearchBar.getValue() ? 26 : 4;

        if (packFolder == null) {
            if (this.searchBar != null) {
                this.searchBar.setVisible(ExileClient.getInstance().getGlobalSettings().showPackSearchBar.getValue());
            }
...
//            this.searchBar.setVisible(false);
    ...
    private void refreshPackFolders() {
        ...
        this.sortPackFolders((String) ExileClient.getInstance().getGlobalSettings().packSortMethod.getValue());
    }

    public void sortAvailableEntries(List<ResourcePackRepository.Entry> list) {
        String sortMethod = (String) ExileClient.getInstance().getGlobalSettings().packSortMethod.getValue();
    ...
    private void searchForPacks(String search) {
        if (search.isEmpty()) {
            ...
            if ((Boolean) ExileClient.getInstance().getGlobalSettings().widePackMenu.getValue()) {
